describe('checa elementos básicos', () => {
  ...
  it('enviar o formulário exibe mensagem de sucesso', () => {
    cy.get('.btn-create-topic').click();
    cy.get('button[type="submit"]').click(); // Selecionando o botão de envio do formulário
    cy.get('.mensagem-sucesso').contains('Aguardando feedback dos autores');
